...
import { TIER_LABELS, type Narration, questionsUpTo } from '@prep/core'
...
/**
 * Lessons live beside their topic in @prep/content, so they are loaded by path.
 * The path is relative rather than aliased because the bundler has to see a
 * static prefix to know which files this can resolve to.
 */
async function loadLesson(technology: string, directory: string) {
  const mdx = await import(
    `../../../../../../../../packages/content/content/${technology}/${directory}/lesson.mdx`
  )
