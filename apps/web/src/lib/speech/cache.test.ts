...
describe('cacheDirectory', () => {
  ...
  it('falls back to a directory git ignores, for a run outside the stack', () => {
    ...
    expect(cacheDirectory()).toBe(join(workspaceRoot(), '.speech-cache'))
  })

  it('anchors a relative setting at the repository root rather than the caller', () => {
    vi.stubEnv('SPEECH_CACHE_DIR', '.speech-cache-e2e')
    expect(cacheDirectory()).toBe(join(workspaceRoot(), '.speech-cache-e2e'))
  })

  /**
   * The app runs from apps/web and the scripts run from the root. Resolving the
   * default against the caller gave them two different cache directories, which
   * is how an e2e run came to assert against recordings it had not emptied.
   */
  it('finds the same root from a workspace member as from the repository root', () => {
    vi.stubEnv('SPEECH_CACHE_DIR', undefined)
    const fromRoot = cacheDirectory()

    const root = workspaceRoot()
    vi.spyOn(process, 'cwd').mockReturnValue(join(root, 'apps', 'web'))
    expect(cacheDirectory()).toBe(fromRoot)
  })
})
