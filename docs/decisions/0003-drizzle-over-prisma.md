# 0003. Drizzle rather than Prisma
...
There is no codegen step either, so no `prisma generate` drift in CI and a
simpler Docker build, and the runtime is smaller. `drizzle-kit studio` covers
the one thing Prisma Studio would have been missed for, a GUI for inspecting
one's own attempt data.
