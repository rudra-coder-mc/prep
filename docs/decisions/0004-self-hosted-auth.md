# 4. better-auth, not Clerk or WorkOS

**Status:** accepted — 2026-08-19

## Context

Auth should not be built from scratch. Hosted providers — Clerk, WorkOS — were
considered as the "just use an existing solution" answer.
...
**Clerk / WorkOS.** Rejected — external dependency and off-server credentials for
no capability gain. Cost was not the deciding factor; both have adequate free
tiers.

**Auth.js (NextAuth v5).** Reasonable and more battle-tested, with a larger
ecosystem. Rejected on DX: its credentials flow is more awkward and v5 has rougher
edges than better-auth's Drizzle integration.
...
Every progress table carries `userId`, so the schema is already multi-user-shaped
— if social login or real multi-user is ever wanted, it is an auth-layer swap
